...
public sealed class QueryRepositoryIntegrationTests : IClassFixture<TestDataEfCoreFixture>, IDisposable
{
    ...
    public QueryRepositoryIntegrationTests(TestDataEfCoreFixture fixture)
    {
        ...
        _repository = new TestQueryRepository(_context, queryParametersOptions, _currentUser);
    }
    ...
    public async Task GetByIdAsync_ShouldReturnEntity(bool isToDisableAggregateIncludes, bool isToDisableEntityTracking)
    {
        ...
        {
            Assert.NotEmpty(_context.ChangeTracker.Entries());
        }
    }
    ...
    public async Task GetByIdAsync_ShouldApplyPermissions(bool isAdmin)
    {
        ...
        if (isAdmin)
        {
            _currentUser.SetCurrentUserRole(UserRoles.Admin);
        }
    ...
    public async Task FirstOrDefaultAsync_ShouldApplyOrderById()
    {
        ...
        // Assert
        Assert.NotNull(result);
        var firstRecordOrderedById = await _context.TestEntities
            .Where(e => e.OwnerId == _currentUser.UserId)
            .OrderBy(e => e.Id)
