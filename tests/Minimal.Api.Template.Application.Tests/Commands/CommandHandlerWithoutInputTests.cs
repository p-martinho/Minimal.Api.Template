...
using Minimal.Api.Template.Application.Tests.TestHandlers;
using NSubstitute;
using NSubstitute.ExceptionExtensions;

namespace Minimal.Api.Template.Application.Tests.Commands;

public class CommandHandlerWithoutInputTests
{
    private readonly TestWithoutInputCommandHandler _commandHandler;
    private readonly ILogger _logger;
    private readonly ITestService _testService;

    public CommandHandlerWithoutInputTests()
    {
        _logger = Substitute.For<ILogger>();

        _testService = Substitute.For<ITestService>();

        _commandHandler = new TestWithoutInputCommandHandler(_logger, _testService);
    }
    ...
    public async Task HandleAsync_ShouldSucceed()
    {
        ...
        // Act
        var result = await _commandHandler.HandleAsync(TestContext.Current.CancellationToken);
